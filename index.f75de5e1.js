...
button.addEventListener("click", (e)=>{
    ...
    if (validateInput(formDataObj)) {
        ...
        for(const key in formDataObj){
            ...
            if (key === "salary") td.textContent = `$${addComa(value)}`;
    ...
    row.addEventListener("dblclick", (e)=>{
        ...
        const saveChanges = ()=>{
            ...
            if (validateField(field, newValue)) {
                if (field === "salary") target.textContent = `$${addComa(newValue)}`;
...
function sortByNumber(rows, pos) {
    rows.sort((a, b)=>{
        const numOne = a.children[pos].textContent.replace(",", "").replace("$", "");
        const numTwo = b.children[pos].textContent.replace(",", "").replace("$", "");
